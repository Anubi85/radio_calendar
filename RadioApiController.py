from Enums import StationTag
import falcon
import json

#TODO: loggare

class RadioApiController:
    def __init__(self, radio):
        self.__radio = radio
    
    @staticmethod
    def __error(error):
        return json.dumps({'error':error})
    @staticmethod
    def __warning(error):
        return json.dumps({'warning':error})
    @staticmethod
    def __get_int_parameter(params, par_name):
        par = params.get(par_name, None)
        if par:
            try:
                par = int(par)
            except:
                par = None
        return par
    @staticmethod
    def __parse_station_data(req):
        data = json.loads(req.stream.read(req.content_length))
        res = len(data) == len(StationTag)
        for tag in StationTag:
            res = res and (tag in data)
        return data if res else None
    
#/stations
    def on_get_stations(self, req, resp):
        stations = self.__radio.on_get_stations()
        resp.body = json.dumps(stations)
    def on_post_stations(self, req, resp):
        data = RadioApiController.__parse_station_data(req)
        if data:
            #check if new position already exists
            pos = data[StationTag.Position]
            if not self.__radio.position_exists(pos):
                #everithing ok, return inserted data
                inserted = self.__radio.add_station(data)                
                resp.body = json.dumps(inserted)
                resp.status = falcon.HTTP_201
            else:
                resp.body = RadioApiController.__error('position {0} already exists'.format(data[StationTag.Position]))
                resp.status = falcon.HTTP_400
        else:
            resp.body = RadioApiController.__error('malformed request content')
            resp.status = falcon.HTTP_400
#/stations/{pos:int}
    def on_get_stations_pos(self, req, resp, pos):
        station = self.__radio.get_station(pos)
        if station:
            resp.body = json.dumps(station)
        else:
            resp.body = RadioApiController.__error('position {0} not found'.format(pos))
            resp.status = falcon.HTTP_404
    def on_put_stations_pos(self, req, resp, pos):
        data = RadioApiController.__parse_station_data(req)
        if data:
            #check if pos is valie
            if self.__radio.position_exists(pos):
                #check if new position exists
                new_pos = data[StationTag.Position]
                if (pos != new_pos) and (not self.__radio.position_exists(new_pos)):
                    old_data = self.__radio.update_station(pos, data)
                    resp.body = json.dumps({'old':old_data,'new':data})
                    radio_update_required = self.__radio.is_playing() and \
                        self.__radio.is_current(new_pos) and \
                        (old_data[StationTag.StreamUrl] != data[StationTag.StreamUrl])
                    if radio_update_required:
                        #update the playing url
                        self.__radio.stop()
                        self.__radio.play()
                else:
                    resp.body = RadioApiController.__error('new position {0} already exists'.format(new_pos))
                    resp.status = falcon.HTTP_400
            else:
                resp.body = RadioApiController.__error('position {0} not found'.format(pos))
                resp.status = falcon.HTTP_404
        else:
            resp.body = RadioApiController.__error('malformed request content')
            resp.status = falcon.HTTP_400
    def on_del_stations_pos(self, req, resp, pos):
        if self.__radio.position_exists(pos):
            deleted = self.__radio.delete_station(pos)
            resp.body = json.dumps(deleted)
            if self.__radio.is_playing() and self.__radio.is_current(pos):
                self.__radio.stop()
        else:
            resp.body = RadioApiController.__error('position {0} not found'.format(pos))
            resp.status = falcon.HTTP_404
#/state
    def on_get_state(self, req, resp):
        state = self.__radio.get_state()
        resp.body = json.dumps(state)
#/control/state/{cmd}
    def on_put_control_state(self, req, resp, cmd):
        if cmd == 'play':
            self.__radio.play()
        elif cmd == 'stop':
            self.__radio.stop()
        else:
            resp.body = RadioApiController.__error('command {0} not supported'.format(cmd))
            resp.status = falcon.HTTP_400
#/control/station/{cmd}
    def on_post_control_station(self, req, resp, cmd):
        new_station = None
        if cmd == 'next':
            new_station = self.__radio.get_next_station()
        elif cmd == 'previous':
            new_station = self.__radio.get_prev_station()
        else:
            resp.body = RadioApiController.__error('command {0} not supported'.format(cmd))
            resp.status = falcon.HTTP_400
            return
        if new_station:
            new_pos = new_station[StationTag.Position]
            self.__radio.set_current(new_pos)
            if self.__radio.is_playing():
                self.__radio.play()
    def on_put_control_station(self, req, resp, cmd):
        PAR_NAME = 'station'
        pos = RadioApiController.__get_int_parameter(req.params, PAR_NAME)
        if cmd == 'set' and pos:
            new_station = self.__radio.get_station(pos)
            if new_station:
                self.__radio.set_current(new_station)
                if self.__radio.is_playing():
                    self.__radio.play()
            else:
                resp.body = RadioApiController.__error('position {0} not found'.format(pos))
                resp.state = falcon.HTTP_404 
        else:
            if pos:
                resp.body = RadioApiController.__error('command {0} not supported'.format(cmd))
            elif PAR_NAME in req.params:
                resp.body = RadioApiController.__error('expected an integer for {0} parameter'.format(PAR_NAME))
            else:
                resp.body = RadioApiController.__error('{0} parameter is missing'.format(PAR_NAME))
            resp.status = falcon.HTTP_400
#/control/volume/{cmd}
    def on_post_control_volume(self, req, resp, cmd):
        PARAM_NAME = 'delta'
        if PARAM_NAME in req.params:
            delta = RadioApiController.__get_int_parameter(req.params, PARAM_NAME)
        else:
            delta = 5
            resp.body = RadioApiController.__warning('{0} parameter is missing, default value (5) is assumed'.format(PARAM_NAME))
        if delta != None:
            if cmd == 'volume-up':
                self.__radio.change_volume(delta)
            elif cmd == 'volume-down':
                self.__radio.change_volume(-delta)
            else:
                resp.body = RadioApiController.__error('command {0} not supported'.format(cmd))
        else:
            resp.body = RadioApiController.__error('expected an integer for {0} parameter'.format(PARAM_NAME))
            resp.state = falcon.HTTP_400
    def on_put_control_volume(self, req, resp, cmd):
        PARAM_NAME = 'value'
        value = RadioApiController.__get_int_parameter(req.params, PARAM_NAME)
        if cmd == 'set' and value:
            self.__radio.set_volume(value)
        else:
            if value:
                resp.body = RadioApiController.__error('command {0} not supported'.format(cmd))
            elif PARAM_NAME in req.params:
                resp.body = RadioApiController.__error('expected an integer for {0} parameter'.format(PARAM_NAME))
            else:
                resp.body = RadioApiController.__error('{0} parameter is missing'.format(PARAM_NAME))
            resp.state == falcon.HTTP_400